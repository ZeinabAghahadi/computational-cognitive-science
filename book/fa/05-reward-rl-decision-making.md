# فصل پنجم: پاداش، یادگیری تقویتی و تصمیم‌گیری

**فارسی** | [English](../en/05-reward-rl-decision-making.md)

[← فصل چهارم](04-language-speech-auditory-models.md) · [فهرست فصل‌ها](README.md) · [نسخهٔ PDF](../../releases/computational-cognitive-science-booklet-fa.pdf)

---

## سیستم پاداش و یادگیری تقویتی

پس از پایان بحث مدل‌های شنیداری، بحث وارد موضوع سیستم‌های پاداش در مغز و ارتباط آن با *Reinforcement Learning* شد. نقطهٔ شروع این بحث این است که یک عامل در محیط قرار دارد، در هر لحظه در یک وضعیت یا *state* است، عملی یا *action* انجام می‌دهد و از یک وضعیت به وضعیت دیگر منتقل می‌شود. به این تغییر وضعیت *transition* گفته می‌شود.

هر *transition* می‌تواند با پاداش یا تنبیه همراه باشد. پاداش ممکن است مثبت، منفی، کم، زیاد یا خنثی باشد. این پاداش‌ها نقش *reinforcer* دارند: باعث می‌شوند عامل یاد بگیرد کدام رفتارها را بیشتر تکرار کند و کدام رفتارها را کم‌تر انجام دهد.

هدف این بخش ورود سنگین به معادلات فیزیکی یا ریاضی نبود. ابتدا قرار بود سازوکار کلی مغز در یادگیری از طریق بازخورد فهمیده شود و سپس این بحث به فضای *Reinforcement Learning* وصل شود.
...
پس از انجام عمل، عامل از زمان $t$ به زمان $t+1$ می‌رود و به وضعیت جدیدی منتقل می‌شود. در همین فاصله، پاداش یا تنبیه دریافت می‌شود. در این نقطه باید چیزی شبیه یک *utility module* یا ماژول ارزش‌گذاری به معماری اضافه شود؛ ماژولی که بررسی کند عمل انجام‌شده خوب بود، بد بود، یا تفاوت خاصی با انتظار قبلی نداشت. هدف این ماژول این است که پس از هر تجربه، مدل قبلی یا *prior model* به‌روزرسانی شود.
...
وقتی دوپامین آزاد می‌شود، در مغز از مسیرهایی عبور می‌کند که به آن‌ها مسیرهای دوپامینرژیک یا *dopaminergic pathways* گفته می‌شود. در بحث پاداش، یکی از نقاط مهم تولید دوپامین ناحیهٔ *Ventral Tegmental Area* یا *VTA* است.

در چارچوب این بخش، کارکرد مهم *VTA* محاسبهٔ خطای پیش‌بینی پاداش است: سیستم چه پاداشی را پیش‌بینی کرده بود، در واقعیت چه پاداشی دریافت شد، و اختلاف این دو چقدر است. وقتی پاداش واقعی بهتر از انتظار باشد، *VTA* با ترشح دوپامین مرتبط می‌شود. بنابراین دوپامین صرفاً نشانهٔ «لذت» نیست؛ بلکه در این بحث بیشتر به غافلگیری مثبت و خطای پیش‌بینی پاداش مربوط است.
...
![مسیرهای دوپامینرژیک مرتبط با VTA](../../assets/figures/dopaminergic-vta-pathways.png)

*شکل ۵ـ۱ — مسیرهای دوپامینرژیک مرتبط با VTA: ارتباط ناحیهٔ تگمنتوم شکمی با هستهٔ اکومبنس، استریاتوم، قشر پیشانی، هیپوکمپ و تودهٔ سیاه.*

#### *Mesolimbic Pathway*

مسیر *Mesolimbic* از *VTA* به سمت سیستم لیمبیک می‌رود. این مسیر با احساس رضایت، شرطی‌شدن، تقویت رفتار، اعتیاد و میل به تکرار رفتار ارتباط دارد.

برای مثال اگر فرد بعد از کار چای بنوشد و این کار برای او تجربهٔ خوشایندی باشد، سیستم *Mesolimbic* می‌تواند به این الگو شرطی شود. در نتیجه، بعد از کار میل به نوشیدن چای افزایش پیدا می‌کند. به زبان ساده، این مسیر به مغز می‌گوید: این رفتار قبلاً خوب بوده است؛ دوباره انجامش بده.

در توضیح این بخش، مسیر *Mesolimbic* از *VTA* به نواحی لیمبیک، به‌ویژه *Nucleus Accumbens*، می‌رسد. این مسیر بیشتر با تقویت یا تضعیف ارزش محرک‌ها در حافظهٔ بلندمدت ارتباط دارد. وقتی خطای پیش‌بینی پاداش مثبت است، این مسیر کمک می‌کند محرک یا عمل مربوطه در حافظه با ارزش بیشتری ثبت شود.

#### *Mesocortical Pathway*

مسیر *Mesocortical* نیز از *VTA* شروع می‌شود، اما به سمت *Prefrontal Cortex* یا قشر پیش‌پیشانی می‌رود. قشر پیش‌پیشانی با تصمیم‌گیری، تحلیل، ارزیابی پیامدها، کنترل رفتار، بررسی ارزش تلاش و فکر کردن به نتایج بلندمدت ارتباط دارد.

مثلاً وقتی فرد کیک می‌بیند، مسیر *Mesolimbic* ممکن است او را به سمت خوردن کیک سوق دهد، چون کیک قبلاً تجربه‌ای لذت‌بخش بوده است. اما مسیر *Mesocortical* پیامدها را ارزیابی می‌کند: آیا خوردن کیک منطقی است؟ آیا فرد گرسنه است؟ آیا رژیم دارد؟ آیا نتیجهٔ بلندمدت این رفتار خوب است؟ در این بیان استعاری، *Mesolimbic* بیشتر نقش گاز و *Mesocortical* بیشتر نقش تحلیل‌گر یا ترمز را دارد.

### اختلال‌های مرتبط با سیستم پاداش
...
در این بحث برای این پرسش که چرا همهٔ افراد وارد چرخهٔ اعتیاد نمی‌شوند، چند عامل مطرح شد. برخی افراد اصلاً در معرض محرکی قرار نمی‌گیرند که خطای مثبت شدید ایجاد کند. آگاهی و محیط نیز مهم‌اند، چون شناخت پیامدهای اعتیاد می‌تواند از ورود به چرخه جلوگیری کند. عامل دیگر تفاوت‌های فردی، ژنتیک و قدرت مسیر *Mesocortical* است. اگر این مسیر در کنترل، راهبرد و ارزیابی پیامدهای بلندمدت قوی‌تر عمل کند، فرد بهتر می‌تواند در برابر پاداش فوری مقاومت کند.

مثال مطرح‌شده *Marshmallow Test* بود: آزمایشی که در آن به کودک گفته می‌شود اگر اکنون شیرینی یا شکلات را نخورد، بعداً پاداش بیشتری می‌گیرد. برخی کودکان می‌توانند صبر کنند و برخی نه. این مثال برای توضیح خودکنترلی و توانایی دیدن پیامدهای بلندمدت استفاده شد.

### ارتباط سیستم پاداش با *Predictive Coding*

در این بخش پرسیده شد کدام نظریهٔ شناختی می‌تواند این فرایند را توضیح دهد. پاسخ اصلی *Predictive Coding* بود. در کدگذاری پیش‌بینانه، سیستم همواره میان دانش قبلی یا *prior knowledge* و واقعیت مشاهده‌شده مقایسه انجام می‌دهد. آنچه برای یادگیری مهم است، صرفاً خود واقعیت خام نیست؛ بلکه خطای میان پیش‌بینی و واقعیت است.
...
مفهوم مرکزی این بخش *Reward Prediction Error* یا خطای پیش‌بینی پاداش است:
$$
...
در ادامه، *Free Energy Principle* مطرح شد. بر اساس این نگاه، انسان فقط به‌دنبال دریافت پاداش بیشتر نیست؛ بلکه تلاش می‌کند عدم قطعیت و خطای پیش‌بینی را کاهش دهد. سیستم شناختی محیط را تجربه می‌کند تا مدل ذهنی‌اش دقیق‌تر شود و اختلاف میان پیش‌بینی و واقعیت کم‌تر شود.
...
## مدل‌های محاسباتی یادگیری تقویتی

از دید محاسباتی، سیستم پاداش انسان شباهت زیادی به *Reinforcement Learning* دارد. در یادگیری تقویتی، عامل در محیط قرار دارد، عملی انجام می‌دهد، از یک وضعیت به وضعیت دیگر می‌رود، پاداش دریافت می‌کند و بر اساس آن پاداش یاد می‌گیرد در آینده چه رفتاری انجام دهد.

### مسئلهٔ *Reinforcement Learning*

مسئلهٔ اصلی *Reinforcement Learning* یادگیری از طریق تعامل با محیط است. عامل برخلاف مدل نظارت‌شده، از ابتدا برچسب درست هر تصمیم را ندارد؛ باید عملی انجام دهد و پیامد آن را ببیند. اگر پاداش دریافت‌شده مطلوب باشد، احتمال تکرار آن رفتار بیشتر می‌شود و اگر پیامد نامطلوب باشد، احتمال آن رفتار کاهش می‌یابد.

یادگیری تقویتی وقتی اهمیت ویژه پیدا می‌کند که محیط آنلاین باشد؛ یعنی عامل مدل آماده و دقیقی از محیط ندارد و باید تجربه را نمونه‌به‌نمونه به‌دست آورد. در هر گام، عامل یک *state* را مشاهده می‌کند، یک *action* انجام می‌دهد، به وضعیت جدیدی منتقل می‌شود و از محیط *reward* یا *punishment* دریافت می‌کند. سپس مدل خود را بر اساس همین تجربه به‌روزرسانی می‌کند.

حالت مهم دیگر، محیط تصادفی یا *stochastic* است. در چنین محیطی، اگر در یک وضعیت مشخص یک عمل مشخص انجام شود، لزوماً همیشه به یک وضعیت مشخص نمی‌رسیم. گذار کلی چنین است:
$$
S \xrightarrow{A} S'
$$
...
### تفاوت با یادگیری نظارت‌شده و بدون‌ناظر

در *Supervised Learning*، داده‌ها برچسب دارند و مدل از رابطهٔ میان *input* و *label* یاد می‌گیرد. اما در سیستم پاداش و یادگیری تقویتی، برچسب آماده وجود ندارد. عامل باید *action* انجام دهد، از محیط *feedback* بگیرد و بر اساس نتیجهٔ تجربه یاد بگیرد.

در *Unsupervised Learning* نیز برچسب مستقیم وجود ندارد، اما مدل معمولاً از ساختار داده‌ها الگو استخراج می‌کند؛ مثلاً خوشه‌بندی می‌کند یا بازنمایی می‌آموزد. تفاوت *Reinforcement Learning* این است که مسئله فقط نبودن برچسب نیست. نکتهٔ اصلی تعامل فعال با محیط و دریافت *reward* پس از عمل است.
...
0 \leq \alpha \leq 1
$$
$\alpha$ همان *learning rate* است. اگر $\alpha = 1$ باشد، فقط تجربهٔ جدید مهم است و دانش قبلی نادیده گرفته می‌شود. اگر $\alpha = 0$ باشد، تجربهٔ جدید اثری ندارد و سیستم چیزی یاد نمی‌گیرد.
...
این منطق *Temporal Difference Learning* نامیده می‌شود، چون اختلاف میان دو زمان را محاسبه می‌کند: پیش از عمل در زمان $t$ و پس از عمل در زمان $t+1$. سیستم ارزش قبلی را به اندازهٔ نرخ یادگیری و بر اساس خطای پیش‌بینی به‌روزرسانی می‌کند.
...
$$
نکتهٔ اصلی این است که در *TD Learning*، در هر مرحله مقدار همان حالتی به‌روزرسانی می‌شود که عامل از آن حرکت کرده است.
...
در *Q-Learning* به‌جای اینکه فقط $V(s)$ به‌روزرسانی شود، مستقیماً $Q(s,a)$ به‌روزرسانی می‌شود:
$$
...
تفاوت مهم *Q-Learning* با *TD Learning* ساده این است که در *Q-Learning* مستقیماً ارزش عمل‌ها در حالت‌ها یاد گرفته می‌شود، نه فقط ارزش خود حالت‌ها. به همین دلیل، *Q-Learning* یک گام به تصمیم‌گیری واقعی نزدیک‌تر است.
...
V(s)=\max_a Q(s,a)
$$
برای انتخاب عمل استفاده کند، همیشه عملی را انتخاب می‌کند که تا آن لحظه بهترین تجربهٔ شناخته‌شده را داشته است. این حالت کاملاً *greedy* یا *exploitative* است؛ یعنی عامل فقط از تجربه‌های قبلی خود استفاده می‌کند.
...
*Exploitation* یعنی عامل از بهترین تجربه‌های قبلی خود استفاده کند: «تا الان این عمل بهترین بوده، پس دوباره همان را انتخاب می‌کنم.» در مقابل، *Exploration* یعنی عامل به عمل‌هایی هم فرصت بدهد که کم‌تر امتحان شده‌اند: «این عمل هنوز بهترین شناخته نشده، اما چون کم‌تر امتحان شده، شاید ارزش کشف کردن داشته باشد.» تصمیم‌گیری در دنیای واقعی معمولاً ترکیبی از این دو است.

### *Passive RL* و *Active RL*

برای اینکه عامل فقط *greedy* نباشد، باید عدم قطعیت را هم وارد تصمیم‌گیری کنیم. اگر امتیاز یک عمل فقط برابر با مقدار $Q$ باشد:
$$
\text{Score}(s,a)=Q(s,a)
$$
...
اگر عملی کم‌تر امتحان شده باشد، عدم قطعیت آن بیشتر است. بنابراین حتی اگر مقدار $Q$ آن فعلاً خیلی بالا نباشد، ممکن است به خاطر عدم قطعیت بالا انتخاب شود تا عامل آن را کشف کند. این ایده به روش‌هایی مانند *Upper Confidence Bound* یا *UCB* شبیه است؛ یعنی عامل فقط میانگین تجربه‌شده را نگاه نمی‌کند، بلکه برای گزینه‌های کم‌تر امتحان‌شده یک حد بالای خوش‌بینانه هم در نظر می‌گیرد.

در حالت *passive*، عامل بیشتر بر اساس تجربه‌های قبلی تصمیم می‌گیرد؛ بهترین گزینه‌های شناخته‌شده را انتخاب می‌کند، تمایل کمی به امتحان گزینه‌های جدید دارد و بیشتر *exploit* می‌کند. در حالت *active*، عامل علاوه بر تجربه‌های قبلی، عدم قطعیت را هم در نظر می‌گیرد؛ گزینه‌های کم‌تر امتحان‌شده را بررسی می‌کند و میان *exploration* و *exploitation* تعادل برقرار می‌کند.

برای توضیح شهودی گفت می‌توان حالت *active* را شبیه جوانی دانست که بیشتر دوست دارد تجربه و کشف کند، و حالت *passive* را شبیه پیری دانست که بیشتر به تجربه‌های قبلی اتکا دارد. این تشبیه برای فهم شهودی است، نه یک ادعای زیستی دقیق.

---

[← فصل چهارم](04-language-speech-auditory-models.md) · [فهرست فصل‌ها](README.md) · [فصل بعد: شبکه‌های اسپایکی و محاسبات نورومورفیک →](06-spiking-neural-networks.md)
